...
def polynomial_mmd_averages(
    codes_g, codes_r, n_subsets=50, subset_size=1000, ret_var=True, **kernel_args
):
...
def test_mpjpe():
    ...
    preds = torch.randn(batch_size, 16, 22, 3)
    targets = torch.randn(batch_size, 16, 22, 3)

    mpjpe = 0
    for i in range(batch_size):
        mpjpe += calc_mpjpe(preds[i], targets[i], align_inds=align_indices)

    jax_mpjpe = gga.metrics.batched_mpjpe(preds.numpy(), targets.numpy()).sum(axis=0)
    chex.assert_trees_all_close(mpjpe.numpy(), jax_mpjpe)


def test_similarity_transform():
    preds = torch.randn(196, 22, 3)
    targets = 0.5 * preds + 5

    sim = batch_compute_similarity_transform_torch(preds, targets)
    jax_sim = gga.metrics.similarity_transform(preds.numpy(), targets.numpy())

    chex.assert_trees_all_close(sim, jax_sim, rtol=1000, atol=1e-6)


def test_pampjpe():
    batch_size = 128

    preds = torch.randn(batch_size, 196, 22, 3)
    targets = 0.5 * preds + 5

    pampjpe = 0
    for i in range(batch_size):
        pampjpe += calc_pampjpe(preds[i], targets[i])

    jax_pampjpe = gga.metrics.batched_pampjpe(preds.numpy(), targets.numpy()).sum(0)
