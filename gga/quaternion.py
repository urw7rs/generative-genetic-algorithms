...
_EPS4 = onp.finfo(float).eps * 4.0

_FLOAT_EPS = onp.finfo(onp.float64).eps


# PyTorch-backed implementations
def qinv(q: jax.Array) -> jax.Array:
    assert q.shape[-1] == 4, "q must be a tensor of shape (*, 4)"
    mask = jnp.ones_like(q)
    mask = mask.at[..., 1:].set(-mask[..., 1:])
    return q * mask
...
def qrot(q, v):
    ...
    qvec = q[..., 1:]
    uv = jnp.cross(qvec, v, axis=-1)
    uuv = jnp.cross(qvec, uv, axis=-1)
